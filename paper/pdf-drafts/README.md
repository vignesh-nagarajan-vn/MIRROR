...
| `MIRROR_Paper_Draft_v9.pdf` | **Current, replacing v8 on SSRN.** Accuracy revision, no new experiments. The post-hoc invariance check is demoted from a headline claim to a regression test ("interpretability at no predictive cost" is gone, since a delta of zero is true by construction). The ablation table no longer prints one AUROC measurement as three results. Both screenshot figures now state that the hosted vision-LLM engine produced them, and the paper says plainly that the grounding guarantee holds for the local PyTorch stack only. New: the operating point (11 of 14 labels emit no positive prediction at threshold 0.5) and a calibration baseline showing a constant-prevalence predictor scores Brier 0.047 against the model's 0.045. Learning rate, resolution, and split counts corrected against the configs. Title block, authors, acknowledgments, and declarations unchanged. Still 8 pages. |
